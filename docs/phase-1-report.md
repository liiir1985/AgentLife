...
| 内核 | `src/config/value-expr.ts` | 受限取值来源（字面量/读取/派生/映射/组合/比较/选择）与比较原语 |
| 内核 | `src/config/conditions.ts` | 受限条件词汇（比较/区间/布尔/显式模拟时间） |
| 内核 | `src/config/system-spec.ts` | `SystemSpec` 与 `SystemCatalog`（系统身份、能力、值集合、兼容性） |
| 内核 | `src/config/value-shapes.ts` | 值/通道形状（Core 提供形状，系统声明拥有权） |
| 内核 | `src/config/rule-catalog.ts` | 规则状态目录：静态输入/输出 + 由定义展开的值集合成员 |
| 内核 | `src/config/system-index.ts` | 系统能力的只读索引（按 `SystemId` 查找） |
| 内核 | `src/config/packs.ts` | root + 依赖包集合、跨命名空间可见性 |
| 内核 | `src/config/source.ts` | 内容包解析（结构阶段）、Markdown 引用展开 |
| 内核 | `src/config/config-merge.ts` | 默认值 → 模板组合 → 显式覆盖，来源追踪 |
| 内核 | `src/config/config-checker.ts` | 阶段 2–7：引用、依赖、组合、权限、系统、兼容性 |
| 内核 | `src/config/config-builder.ts` | 不可变 `RuntimeConfig`、触发索引、稳定顺序、组合计划、`configId` |
| 内核 | `src/config/rule-engine.ts` | 确定性运行、变化请求与完整追踪 |
| 内核 | `src/config/core-runtime.ts` | 配置发布、当前版本、恢复核对、`runRules()` 入口 |
| 内核 | `src/config/demo-runner.ts` | 演示运行器：打印配置身份、索引、逐目标组合值与追踪摘要 |
| 系统 | `src/systems/world-spec.ts`、`body-spec.ts`、`character-spec.ts`、`index.ts` | 世界/身体/人物的配置类型、值集合、输入、触发器、状态输出与系统检查 |
| 存储 | `src/storage/migrations.ts` | 版本化迁移（`PRAGMA user_version`） |
| 存储 | `src/storage/runtime-store.ts` | 当前运行配置、版本历史、运行追踪、效果消费身份、恢复前置检查 |
...
| 测试 | `tests/rule-kernel.test.ts` | 数值策略、映射、`ValueExpr`、条件、系统装载与值集合声明 |
| 测试 | `tests/configuration-validation.test.ts` | 35 项拒绝用例 + 4 项合成系统说明的解析与组合用例 |
...
| 规则文档 | `{ kind: rule, id, system, triggers, inputs?, condition, changes, dependsOn? }`；`inputs` 每项为 `{name, state}`，条件和变化表达式通过 `name` 取值。 |
| 公式文档 | `{ kind: formula, id, system, inputs?, outputUnit, value }`；输出单位必须与 `ValueExpr` 的单位一致。 |
| 长文本 | 任何 `.md` 字符串都被解析为受控引用并替换为文件正文（阶段 0 已保证引用存在且在包内）；引用参与内容包完整性身份。 |
| 未知字段 | 任何未在 `SystemSpec` 中出现的字段都是结构错误（不静默忽略）。 |
...
- **不受支持语义**：Schema 出现白名单外构造（如 `Type.Any()`）、默认值与字段类型不符、对非数组字段声明 `append`、对非对象字段声明 `merge`、值集合成员声明了不存在的字段或类型无法容纳该成员、字符串状态声明词表以外的值类型、同一命名空间内跨类别重名（**输入与输出同名除外**：它们是同一状态的读侧与写侧）。
- **越权能力**：输入或输出声明 `exposedTo: ["*"]`（Core 不授予通配可见性）、声明不支持的输出值类型。
- **身份冲突**：同一 `SystemId` 装载了不同内容；相同内容重复装载是幂等的。
- **语义不兼容**：Core 版本区间不满足，或 `requires` 的同伴系统缺失/版本不符（`finalize()` 统一检查，**与装载顺序无关**）。

暴露名单可以写尚未装载的系统：`finalize()` 对这类授权给出警告而不是拒绝，装载后自然生效。

`specHash`（系统说明规范化 JSON 的 SHA-256）进入 `configId`：系统语义变化必然产生新的运行配置版本。
...
| 阶段 | 实现位置 | 覆盖内容 |
| --- | --- | --- |
| 1 结构 | `source.ts`、`config-merge.ts`、`system-spec.ts`、`rule-catalog.ts` | 必需组成、类型、枚举、未知字段、默认值/合并策略合法性、值集合成员与定义字段/类型的一致性、定义 id 可引用 |
| 2 引用 | `config-merge.ts`、`config-checker.ts#bindInputs/checkItemReferences` | 模板与字段引用存在性、类型匹配、跨命名空间可见性、规则/公式存在性、状态字段与值成员存在性 |
| 3 依赖 | `config-checker.ts#dependencyOrder` | 规则与公式的拓扑顺序，环即拒绝，同级按稳定 ID 排序 |
| 4 组合 | `config-checker.ts#checkCombinePlan/checkValueCompatibility/checkValueUnits` | 同一目标组合方式一致、**契约值恰好一个写方**、优先级声明、单位与值类型兼容、映射策略完整性、组合/比较/选择内部一致性 |
| 5 权限 | `config-checker.ts#bindInputs/bindChanges` | 状态读取授权、状态变化授权、过程动作仅限拥有系统、触发器存在性 |
| 6 系统 | `config-checker.ts#checkSystems` + 各 `SystemSpec.validate` | 系统词汇与不变量（见 §7） |
| 7 兼容性 | `config-checker.ts#checkCompatibility` + `SystemCatalog.finalize` | 包与 Core 的版本、系统版本固定、同伴系统要求 |

任一阶段出现 error 即整体拒绝，**当前有效配置完全不变**（`publish()` 返回 `rejected` + 诊断，`CoreRuntime.current()` 仍指向旧版本，测试以对象同一性断言）。

诊断携带 `severity/stage/code/message/subject/source{pack,file,path}`；作用域覆盖 §20 验收场景 3、4、6、8、9、31、32 所需的全部拒绝路径。

## 6. 编译产物与配置身份（§9.3、§5.2）

`buildConfig()` 产出深冻结的 `RuntimeConfig`：固定的引用与类型、`triggerIndex`（触发器 → 选中规则，稳定顺序）、规则与公式的稳定运行顺序、输入集合与状态输出集合、每个 `StateRef` 的 `CombinePlan`（含值类型、可选数值策略、可选字符串词表）、包/系统身份元数据，以及可重建配置的 `sourceData`。

运行配置身份 = 规范化 JSON 的 SHA-256，输入包含：

```
kernelVersion + systems[{systemId, version, specHash}] + packs[{namespace, version, kernel, contentId}]
+ 定义[{ref, type, isPublic, values, fields{merge, contributions}}]
+ 规则[{ref, system, triggers, inputs, usedInputs, condition, changes, dependsOn, order}]
+ 公式[{ref, system, inputs, usedInputs, outputUnit, value, order}]
+ triggerIndex + combinePlans
```

同一内容重复 `publish()` 得到相同 `configId`；改一个地点名、改一条规则或改 `SystemSpec` 都会得到新 ID（测试覆盖前两者，`specHash` 在系统目录测试中覆盖）。改一个**未参与规则运行**的字段同样改变 ID——它标识的是完整内容，而非抽样语义。
...
1. 请求携带的 `baseVersion` 与快照 `stateVersion` 不一致 → `state-version-stale`，一条规则都不执行（§15.3）。
2. 触发器索引选中规则（无命中 → `no-match`）；未命中规则在追踪里列入 `notIndexed`，以区分「没有被触发」。
3. 逐条规则：声明过的输入全部存在才继续（否则 `input-missing`）；运行条件（失败 → `input-invalid`/`input-missing`；false → `condition-false`）；计算变化值。
4. 按 `StateRef` 分组并按 `CombinePlan` 合并，产生 `StateChangeRequest`；过程动作单独形成 `ProcessChangeRequest`。
5. 结果状态按固定优先级汇总（见下）。
...
| `input-invalid` | 存在缺失/类型不符/单位不符的输入（变化请求可能只形成了一部分） |
| `inexpressible` | 组合结果超出目标声明的表示范围（`overflow: reject`）或落在字符串词表之外 |
| `conflict` | 相同最高优先级给出不同值，不静默择一 |
| `changes` | 得到完整、无歧义的变化请求集合 |
| `condition-false` | 命中规则均不满足条件 |
| `no-match` | 触发索引没有任何规则 |

**优先级理由**：不完整运行（`input-invalid`）优先于 `changes`，避免调用方把部分请求当成完整结果；`inexpressible`/`conflict` 优先于 `changes`，两者都不能直接应用。追踪仍逐条保留原因，因此优先级只影响汇总状态，不丢信息。
...
同一 `StateRef` 的 `CombineMode` 必须一致：混用不同方法在加载期即被拒绝（`missing-combine`），运行期才出现的并列冲突返回结构化冲突而不会择一。**组合方式由规则声明，`SystemSpec` 不再声明允许的方法集合**；对标记为契约值的成员，加载期额外要求**恰好一个写方**（`multiple-writers`），因为核心值不允许出现“谁写了它”的歧义。
...
`CoreRuntime.publish` 的顺序：解析 → 打包集合 → 7 阶段验证 → 编译 → **持久化提交** → 切换内存中的当前版本。失败路径：
...
| 测试文件 | 用例 | 保护的不变量 |
| --- | --- | --- |
| `tests/rule-kernel.test.ts` | 26 | 舍入/边界/溢出、策略块缺省语义、三种映射与区间外行为、`ValueExpr`（读/映射/组合/比较/选择）、条件语义、系统装载与值集合声明拒绝路径 |
...
端到端路径（`tests/configuration-registry.test.ts`、`tests/configuration-evaluation.test.ts`、`src/config/demo-runner.ts`）用的都是仓库内的真实演示内容包 `content/demo`：读取 → 解析 → 合并 → 检查 → 构建 → 发布 → 运行规则。负例用“复制演示包并覆盖单个文件”的方式构造，保证拒绝路径面对的是真实内容而不是手写的残缺文档；未随包发布的过程能力用合成 `SystemSpec` `test.process` 覆盖。
...
| 3 缺失引用、跨命名空间越权、包依赖环、不兼容系统语义生效前拒绝 | 已覆盖（跨命名空间两例、模板环、Core 版本、系统版本） |
| 4 `SystemSpec` 可增加能力但不能越权读写 | 已覆盖（未授权读取/写入/过程动作、通配可见性拒绝） |
...
1. **一切状态变更请求经内核产生、由领域提交**：阶段 2 的 WorldService/BodyService 只能消费 `StateChangeRequest`（含 `baseVersion`），提交后把领域状态版本回填给下一次求值。
2. **效果身份用于防重**：提交前用 `consumeEffect(effectId, timelineId, configIdentity)` 领取；返回 `duplicate` 必须跳过，这是崩溃恢复不重复应用的唯一依据。
3. **Tick 只查索引**：用 `triggerIndex` 选择候选规则，禁止遍历全部规则；触发器词汇必须先在系统说明中声明。
