...
命令：`pnpm spike:tui` → `tests/tui-spike.test.ts`，18 passed。
...
| Pi TUI | 宽窄布局、缩放、日志滚动、Overlay、焦点自动测试通过 | **通过**（自动：TUI 18 项 + 演示脚本 5 项；人工：`src/tui/manual-spike.ts` 清单在 Windows Terminal 逐条通过，见 §6.3） |
...
pnpm test           6 files, 59 passed | 1 skipped（含 TUI 18 项与演示脚本 5 项）
```
...
11. **鼠标默认不捕获（框选/右键交给终端）**：OMP 的 `tui.mouse` 默认 `false`，主会话不发送鼠标跟踪序列，因此拖拽框选、右键菜单/粘贴、清除选择全部是 Windows Terminal 自己的行为；打开该设置后提示语明确写着"原生框选退化为 Shift+拖拽、滚轮退化为 Shift+滚轮"。修法：`SpikeApp` 的 `mouse` 默认 `false`（`pnpm tui:manual` 默认不捕获，加 `--mouse` 才由应用接管），自动测试断言默认构造写出的字节不含 `\x1b[?1006h`、`mouse: true` 时含。**遗留**：pi-tui 没有公开的"清除文本选择"API（`clearTextSelection` 只挂在鼠标按下派发路径上），所以 `--mouse` 模式下应用自绘的选择高亮只能靠左击清除，且 `copyOnSelect` 默认 `true`（松手即复制并闪一次 "Copied"）；右键粘贴回调 `onRightClickPaste` 只在 Windows 且**捕获鼠标时**才有事件——不捕获时由终端负责粘贴（WT 的粘贴同样以 `\x1b[200~…\x1b[201~` 投递，走的就是第 10 条那条路径，已实测）。
...
- **鼠标默认交由终端（这是选择，不是缺陷）**：`SpikeApp`/`pnpm tui:manual` 默认 `mouse: false`，框选、右键、清除选择都是 Windows Terminal 自己的行为（与 OMP 的 `tui.mouse` 默认值一致）；代价是应用收不到滚轮与悬停，滚轮行为由终端决定（alt-screen 内可能被转成方向键或忽略），日志翻页用 `PageUp/PageDown`/`Ctrl+Home`/`Ctrl+End`（已自动测试）。`--mouse` 打开应用接管：滚轮滚动日志、卡片悬停高亮、选择由应用自绘，但原生框选退化为 Shift+拖拽、松手即复制（`copyOnSelect` 默认 `true`）、高亮只能靠左击清除（pi-tui 无公开 clear API），且右键粘贴回调只在捕获鼠标时才有事件（仅 Windows）。
...
- TUI 输入层的既有约定（阶段 1 必须保持）：Home/End 归输入行（`tui.altScreen.top/bottom` 改绑 `ctrl+home`/`ctrl+end`）；**多行/超长粘贴走 chip（token + 卡片 + 提交展开），单行短粘贴内联且换行压成空格，粘贴内容先规范化（CRLF→LF、tab→4 空格、剔除除 LF 外的所有 C0 控制字符）**；右键粘贴走 `onRightClickPaste` + `getNativeClipboard()`（仅在捕获鼠标时有事件）；**默认不捕获鼠标，框选/右键/滚轮交给终端**，需要应用滚轮与悬停时才开 `mouse`；不得让新监听抢在输入栏之前消费这些按键。
