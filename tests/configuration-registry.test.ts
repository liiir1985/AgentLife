...
import { ConfigurationRegistry, contentPackInput } from "../src/config/registry.js";
...
`,
    ...
    try {
      ...
      const identity = first.result.config?.identity ?? "";
      expect(identity).toMatch(/^[0-9a-f]{64}$/);

      expect(second.result.config?.identity).toBe(identity);

      expect(changed.result.status).toBe("valid");
      expect(changed.result.config?.identity).not.toBe(identity);
    } finally {
  ...
  it("resolves defaults, template composition and overrides with visible provenance", async () => {
    const { result, directory } = await applyDemoPack(createRegistry());
    try {
      const companion = result.config?.definitions.find((definition) => definition.ref === "agentlife.demo/companion");
      ...
      expect(modules?.contributions.map((contribution) => contribution.layer)).toEqual(["default", "template"]);
      expect(modules?.contributions.map((contribution) => contribution.source)).toEqual([
      ...
      expect(identity?.contributions.map((contribution) => contribution.layer)).toEqual(["template", "override"]);
      // The override is a markdown reference, resolved before the schema check.
      expect(String(identity?.value)).toContain("认得每一棵树的脾气");

      const baseline = result.config?.definitions.find(
        (definition) => definition.ref === "agentlife.demo/normal-entity",
      );
      expect(baseline?.fields.name?.contributions.map((contribution) => contribution.layer)).toEqual(["override"]);

      const player = result.config?.definitions.find((definition) => definition.ref === "agentlife.demo/player");
      expect(player?.fields.control?.contributions.map((contribution) => contribution.layer)).toEqual([
        "template",
        "override",
      ]);
  ...
  it("exposes a value family as one view whose members are the declared values", async () => {
    const { result, directory } = await applyDemoPack(createRegistry());
    try {
      const stamina = result.config?.definitions.find((definition) => definition.ref === "agentlife.demo/stamina");
      ...
      // A channel contributes two members from one definition.
      const vision = result.config?.definitions.find((definition) => definition.ref === "agentlife.demo/vision");
      expect(vision?.values.available).toBe(true);
      expect(vision?.values.efficiency).toBe(0.8);
      const plans = Object.keys(result.config?.compositionPlans ?? {}).sort();
  ...
  it("resolves long text through the pack's markdown reference", async () => {
    const { result, directory } = await applyDemoPack(createRegistry());
    try {
      const world = result.config?.definitions.find((definition) => definition.ref === "agentlife.demo/valley");
      ...
      const square = result.config?.definitions.find(
        (definition) => definition.ref === "agentlife.demo/lantern-square",
      );
...
`,
  ...
  it("never publishes a version whose persistence failed before the commit", async () => {
    ...
    const persistence = {
      saveRuntimeConfig: (config: { identity: string; document: unknown }) => {
        throw new Error(`injected write failure for ${config.identity.length} bytes`);
      },
      currentRuntimeConfig: () => {
        const [identity] = [...durable.keys()];
        return identity === undefined ? undefined : { identity, document: durable.get(identity) };
      },
      configDocument: (identity: string) => durable.get(identity),
    };
    const registry = new ConfigurationRegistry(undefined, persistence);
    for (const extension of createDomainExtensions()) registry.registerExtension(extension);
  ...
  it("adopts a version that was committed even when the acknowledgement was lost", async () => {
    ...
    const persistence = {
      saveRuntimeConfig: (config: { identity: string; document: unknown }) => {
        durable.set(config.identity, config.document);
    ...
    try {
      ...
      expect(registry.current()?.identity).toBe(result.config?.identity);
    } finally {
      removeDirectory(directory);
    }
  });
  ...
  it("never publishes a version the durable state did not accept", async () => {
    const persistence = {
      ...
      currentRuntimeConfig: () => ({ identity: "0".repeat(64), document: {} }),
  ...
  it("rebuilds a stored version from its own document and refuses to substitute another", async () => {
    ...
    const accepted = await applyDemoPack(registry);
    try {
      const identity = accepted.result.config?.identity ?? "";
      const restored = registry.restore(identity);
      expect(restored.status).toBe("valid");
      expect(restored.config?.identity).toBe(identity);
  ...
  it("refuses a foreign definition the pack never depended on or that stays private", async () => {
    await withTempDirectory(async (directory) => {
      ...
      const dependencyPack = contentPackInput(await loader.load(dependency));
      ...
      const scenarios: readonly { readonly name: string; readonly dependencies: readonly string[] }[] = [
        ...
        { name: "private definition", dependencies: ["agentlife.library"] },
      ];
      for (const scenario of scenarios) {
        ...
        const applied = createRegistry().apply({
          root: contentPackInput(await loader.load(root)),
  ...
  it("accepts a published definition from a declared dependency", async () => {
    await withTempDirectory(async (directory) => {
      const dependency = path.join(directory, "library");
      const root = path.join(directory, "host");
      const loader = new ContentPackLoader();
      ...
      const applied = createRegistry().apply({
        root: contentPackInput(await loader.load(root)),
        dependencies: [contentPackInput(await loader.load(dependency))],
      });
      ...
      const plaza = applied.config?.definitions.find((definition) => definition.ref === "agentlife.host/plaza");
